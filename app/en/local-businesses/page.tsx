...
import { getPublicEnglishUmkmPath } from "@/config/public-routes";
...
export const metadata = buildPublicMetadata({
  title: ENGLISH_UMKM_COPY.list.metadataTitle,
  description: ENGLISH_UMKM_COPY.list.metadataDescription,
...
function UmkmCard({
  umkm,
  highPriority,
}: Readonly<{ umkm: PublicEnglishUmkm; highPriority: boolean }>) {
  return (
    <Link
      href={getPublicEnglishUmkmPath(umkm.slug)}
    ...
    >
      <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow group-hover:shadow-lg group-focus-visible:shadow-lg motion-reduce:transition-none">
        <PublicMediaImage
          src={umkm.primaryImage?.signedUrl ?? null}
          alt={umkm.primaryImage?.altText ?? umkm.businessName}
        ...
        <div className="flex flex-1 flex-col p-5 sm:p-6">
          ...
          <p className="mt-5 text-sm font-bold text-emerald-800 group-hover:text-emerald-950">
            {ENGLISH_UMKM_COPY.list.cardAction}{" "}
...
export default async function EnglishUmkmListPage() {
  const [result, contactResult] = await Promise.all([
    getPublishedEnglishUmkms(),
    getEnglishPublicShellData(),
  ]);
  if (result.kind === "error") {
    throw new Error("PUBLIC_ENGLISH_UMKM_LIST_UNAVAILABLE");
  }
  return (
      ...
      <section className="border-b border-emerald-950/10 bg-emerald-950 py-16 text-white sm:py-20">
        <PublicContainer>
          <p className="text-sm font-bold tracking-[0.18em] text-amber-300 uppercase">
            {ENGLISH_UMKM_COPY.list.eyebrow}
          </p>
          <h1 className="mt-4 max-w-4xl font-serif text-4xl font-bold tracking-tight sm:text-5xl">
            {ENGLISH_UMKM_COPY.list.title}
          </h1>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-emerald-50/80">
            {ENGLISH_UMKM_COPY.list.description}
          </p>
        </PublicContainer>
      </section>
      <section className="py-12 sm:py-16" aria-labelledby="english-umkms">
        <PublicContainer>
          <div className="border-b border-slate-200 pb-8">
            <p className="text-sm font-bold tracking-[0.14em] text-emerald-800 uppercase">
              {ENGLISH_UMKM_COPY.list.sectionEyebrow}
            </p>
            <h2
              id="english-umkms"
              className="mt-2 font-serif text-3xl font-bold"
            >
              {ENGLISH_UMKM_COPY.list.sectionTitle}
            </h2>
          </div>
          <div className="mt-8">
            {result.umkms.length === 0 ? (
              <EmptyContentState
                title={ENGLISH_UMKM_COPY.list.emptyTitle}
                description={ENGLISH_UMKM_COPY.list.emptyDescription}
              />
            ) : (
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {result.umkms.map((umkm, index) => (
                  <UmkmCard
                    key={umkm.id}
                    umkm={umkm}
