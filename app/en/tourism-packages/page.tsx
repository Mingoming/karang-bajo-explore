...
import { getPublicEnglishTourismPackagePath } from "@/config/public-routes";
...
export const metadata = buildPublicMetadata({
  title: ENGLISH_TOURISM_PACKAGE_COPY.list.metadataTitle,
  description: ENGLISH_TOURISM_PACKAGE_COPY.list.metadataDescription,
  openGraphLocale: "en_US",
});

function EnglishTourismPackageCard({
  tourismPackage,
  highPriority,
}: Readonly<{
  ...
  if (!tourismPackage.primaryImage) return null;

  return (
    <Link
      href={getPublicEnglishTourismPackagePath(tourismPackage.slug)}
      aria-label={`${ENGLISH_TOURISM_PACKAGE_COPY.list.cardAction}: ${tourismPackage.name}`}
      ...
      <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow group-hover:shadow-lg group-focus-visible:shadow-lg motion-reduce:transition-none">
        ...
        <div className="flex flex-1 flex-col p-5 sm:p-6">
          ...
          <p className="mt-4 text-sm font-semibold text-slate-700">
            {tourismPackage.durationValue} {tourismPackage.durationUnit} ·{" "}
          ...
          <p className="mt-5 text-sm font-bold text-emerald-800 group-hover:text-emerald-950">
            {ENGLISH_TOURISM_PACKAGE_COPY.list.cardAction}{" "}
...
export default async function EnglishTourismPackageListPage() {
  ...
  if (result.kind === "error") {
    throw new Error("PUBLIC_ENGLISH_TOURISM_PACKAGE_LIST_UNAVAILABLE");
  }
        ...
        <PublicContainer>
          ...
          <div className="mt-8">
            {result.packages.length === 0 ? (
              <EmptyContentState
                title={ENGLISH_TOURISM_PACKAGE_COPY.list.emptyTitle}
                description={ENGLISH_TOURISM_PACKAGE_COPY.list.emptyDescription}
              />
            ) : (
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {result.packages.map((tourismPackage, index) => (
                  <EnglishTourismPackageCard
