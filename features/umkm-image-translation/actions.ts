...
import {
  queryUmkmImageTranslationAdminData,
  type UmkmImageTranslationAdminReadResult,
} from "./data";
import {
  createUmkmImageTranslationActionState,
  validateUmkmImageTranslationForEligibility,
  validateUmkmImageTranslationForSource,
  validateUmkmImageTranslationFormData,
  type UmkmImageTranslationActionState,
  type UmkmImageTranslationMutationValues,
  type UmkmImageTranslationRpcRow,
} from "./model";
...
type SuccessfulRead = Extract<
  UmkmImageTranslationAdminReadResult,
  { success: true }
>;
...
type StateOverrides = {
  kind?: UmkmImageTranslationActionState["kind"];
  values?: UmkmImageTranslationActionState["values"];
  fieldErrors?: UmkmImageTranslationActionState["fieldErrors"];
  formErrors?: string[];
  message?: string | null;
  rejectionReason?: string;
  sourceStatus?: UmkmImageTranslationActionState["sourceStatus"];
};
...
function stateFromRead(
  ...
  return createUmkmImageTranslationActionState(
    image.source,
    image.translation,
    image.history,
    {
...
function databaseFailureState(
  previousState: UmkmImageTranslationActionState,
  message = "Terjemahan gambar umkm belum dapat diproses. Silakan coba lagi.",
  kind: UmkmImageTranslationActionState["kind"] = "database-error",
) {
  return {
    ...previousState,
    kind,
...
function rpcFailureState(
  ...
  let message =
    "Terjemahan gambar umkm belum dapat diproses. Silakan coba lagi.";
  let kind: UmkmImageTranslationActionState["kind"] = "database-error";
  if (code === "55000") {
    kind = "conflict";
    message =
...
function revalidateUmkmImageTranslationPaths(
  umkmId: string,
  sourceSlug: string,
) {
...
async function refreshAfterMutation(
  ...
  if (refreshed.slug !== trustedPreMutationSlug) {
    revalidateUmkmImageTranslationDetailPath(refreshed.slug);
  }
...
function rpcRowFailureCode(
  ...
  row: UmkmImageTranslationRpcRow | null,
) {
...
export async function manageUmkmImageTranslation(
  umkmId: string,
  imageId: string,
  previousState: UmkmImageTranslationActionState,
  formData: FormData,
): Promise<UmkmImageTranslationActionState> {
  ...
  if (!isValidUmkmId(umkmId) || !isValidUmkmId(imageId)) {
    return databaseFailureState(
      previousState,
      "Umkm atau gambar tidak ditemukan.",
      "not-found",
    );
  }
  ...
  const current = await queryUmkmImageTranslationAdminData(supabase, umkmId);
  if (!current.success) {
    return databaseFailureState(
      previousState,
      current.kind === "not-found"
        ? "Umkm tidak ditemukan."
        : "Terjemahan gambar umkm belum dapat dimuat. Silakan coba lagi.",
  ...
  if (!image) {
    return databaseFailureState(
      previousState,
      "Gambar tidak ditemukan atau bukan bagian dari umkm ini.",
      "not-found",
    );
  }
  ...
  if (intent === "save-draft" || intent === "review") {
    const validation = validateUmkmImageTranslationFormData(formData);
    ...
    const sourceRule = validateUmkmImageTranslationForSource(
      image.source,
      validation.data,
    );
    ...
    if (intent === "review") {
      ...
      const eligibility = validateUmkmImageTranslationForEligibility(
        image.source,
        validation.data,
        image.sourceStatus,
      );
    ...
    if (intent === "save-draft") {
      return refreshAfterMutation(
        ...
        current.slug,
        previousState,
        "Draf terjemahan gambar umkm berhasil disimpan.",
      );
    }
    const reviewed = await supabase
      .rpc("umkm_image_translation_review", {
    ...
    return refreshAfterMutation(
      ...
      current.slug,
      previousState,
      "Terjemahan gambar umkm berhasil dikirim untuk review.",
    );
  }
  ...
  if (intent === "reject") {
    ...
    const rejected = await supabase
      .rpc("umkm_image_translation_reject", {
    ...
    return refreshAfterMutation(
      ...
      current.slug,
      previousState,
      "Terjemahan gambar umkm dikembalikan menjadi draf dengan alasan penolakan.",
    );
  }
  if (intent === "publish" || intent === "republish") {
    ...
    return refreshAfterMutation(
      ...
      previousState,
      intent === "publish"
        ? "Terjemahan gambar umkm berhasil diterbitkan."
        : "Terjemahan gambar umkm berhasil diterbitkan kembali.",
    );
  }
  ...
  return refreshAfterMutation(
    ...
    previousState,
    intent === "archive"
      ? "Terjemahan gambar umkm berhasil diarsipkan."
      : intent === "unpublish"
        ? "Publikasi terjemahan gambar umkm dibatalkan dan kembali menjadi draf."
        : "Terjemahan gambar umkm dipulihkan menjadi draf.",
  );
}
