...
import { manageUmkmImageTranslation } from "./actions";
import {
  getUmkmImageTranslationLifecycleLabel,
  isUmkmImageTranslationEditable,
  type UmkmImageTranslationActionState,
  type UmkmImageTranslationField,
  type UmkmImageTranslationSource,
} from "./model";

type Props = {
  umkmId: string;
  initialState: UmkmImageTranslationActionState;
  sourceReference: UmkmImageTranslationSource;
};

const FIELD_CONFIG: Array<{
  field: UmkmImageTranslationField;
  label: string;
  rows: number | null;
}> = [
...
function statusClasses(
  state: UmkmImageTranslationActionState["lifecycleState"],
) {
  if (state === "published")
...
export function UmkmImageTranslationForm({
  umkmId,
  initialState,
  sourceReference,
}: Props) {
  const boundAction = manageUmkmImageTranslation.bind(
    null,
    umkmId,
    sourceReference.id,
  );
  ...
  const lifecycleLabel = getUmkmImageTranslationLifecycleLabel(
    state.lifecycleState,
  );
  const editable = isUmkmImageTranslationEditable(
    state.status,
    state.reviewState,
  );
  const canReview =
    ...
    <section className="mt-8 border-t border-slate-200 pt-8">
      ...
      {state.lifecycleState === "source-blocked" ? (
        <div
          ...
          className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950"
        >
          Sumber umkm belum memenuhi kelayakan publik. Gambar Inggris tetap
      ...
      <dl className="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-3">
        <div>
          ...
          <dd className="mt-1 font-mono text-xs">
            {sourceReference.sourceSlug}
          </dd>
        </div>
        <div>
          <dt className="font-semibold text-slate-800">Source revision</dt>
          <dd className="mt-1">{sourceReference.sourceRevision}</dd>
        </div>
        <div>
          <dt className="font-semibold text-slate-800">Source updated</dt>
          <dd className="mt-1">{sourceReference.sourceUpdatedAt}</dd>
        </div>
      </dl>
      {state.sourceBlockedReason ? (
        ...
        >
          ...
          >
            ...
            {FIELD_CONFIG.map(({ field, label, rows }) => {
              const messageId = `umkm-image-translation-${field}-message`;
              const required = field === "alt_text";
              return (
                <div key={field}>
                  <label
                    htmlFor={`umkm-image-translation-${field}`}
                  ...
                  {rows === null ? (
                    <input
                      id={`umkm-image-translation-${field}`}
                      name={field}
                      type="text"
          ...
          {canReject ? (
            <div>
              <label
                htmlFor="umkm-image-translation-rejection-reason"
                className="block text-sm font-semibold text-slate-800"
              >
                Alasan penolakan{" "}
                <span aria-hidden="true" className="text-red-700">
              ...
              <textarea
                id="umkm-image-translation-rejection-reason"
