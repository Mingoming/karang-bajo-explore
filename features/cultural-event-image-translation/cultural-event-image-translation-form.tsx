...
import { manageCulturalEventImageTranslation } from "./actions";
import {
  getCulturalEventImageTranslationLifecycleLabel,
  isCulturalEventImageTranslationEditable,
  type CulturalEventImageTranslationActionState,
  type CulturalEventImageTranslationField,
...
type Props = {
  ...
  initialState: CulturalEventImageTranslationActionState;
  sourceReference: CulturalEventImageTranslationSource;
};

const FIELD_CONFIG: Array<{
  field: CulturalEventImageTranslationField;
  label: string;
  rows: number | null;
}> = [
  { field: "alt_text", label: "English alt text", rows: null },
  { field: "caption", label: "English caption", rows: 3 },
];

function statusClasses(state: CulturalEventImageTranslationActionState) {
...
function formatSourceInstant(value: string) {
...
export function CulturalEventImageTranslationForm({
  culturalEventId,
  sourceContext,
  initialState,
  sourceReference,
}: Props) {
  const boundAction = manageCulturalEventImageTranslation.bind(
    null,
    culturalEventId,
    sourceReference.id,
  );
  ...
  const editable = isCulturalEventImageTranslationEditable(
    state.status,
    state.reviewState,
  );
  const lifecycleLabel = getCulturalEventImageTranslationLifecycleLabel(
    ...
    <section className="mt-8 border-t border-slate-200 pt-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-xl font-bold tracking-tight text-slate-950">
            Image {sourceReference.displayOrder}
            {sourceReference.isPrimary ? " — primary image" : ""}
          </h3>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
            Indonesian media metadata is read-only reference. This form edits
            translation metadata only; it never uploads, replaces, reorders, or
            deletes Storage media.
          </p>
        </div>
        <span
      ...
      {state.lifecycleState === "stale" ? (
        ...
        >
          ...
          <p className="mt-1">
            A source media or translation change invalidated the review
            checkpoint. Fresh review and explicit publication are required.
          </p>
        </div>
      ) : null}
      {state.sourceBlocked ? (
        <div
          ...
          className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950"
        >
          The Indonesian source is not currently eligible for public English
          media. The database remains the authority.
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <aside className="rounded-xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
          ...
          <dl className="mt-6 border-t border-slate-200 pt-5 text-sm leading-6 text-slate-900">
            <div className="flex flex-wrap justify-between gap-x-4 gap-y-1">
              <dt className="font-semibold text-slate-700">
                Parent source slug
              </dt>
              <dd className="break-all">{sourceContext.slug}</dd>
            </div>
            <div className="mt-2 flex flex-wrap justify-between gap-x-4 gap-y-1">
              <dt className="font-semibold text-slate-700">
                Parent source updated (WITA)
              </dt>
              <dd>{formatSourceInstant(sourceContext.updatedAt)}</dd>
            </div>
          </dl>
        </aside>
        ...
        >
          ...
          >
            <legend className="text-lg font-bold text-slate-950">
              English image fields
            </legend>
            <p className="text-sm leading-6 text-slate-600">
              English alt text is required for review and publication. Caption
              is optional, but must remain empty when the source caption is
              empty.
            </p>
            {FIELD_CONFIG.map(({ field, label, rows }) => {
              const required = field === "alt_text";
              const messageId = `cultural-event-image-translation-${field}-message`;
              return (
                <div key={field}>
                  <label
                    htmlFor={`cultural-event-image-translation-${field}`}
                  ...
                  {rows === null ? (
                    <input
                      id={`cultural-event-image-translation-${field}`}
                      name={field}
                      type="text"
                  ...
                  >
                    {state.fieldErrors[field] ??
                      (required
                        ...
                        : "Optional; keep empty when the source caption is empty.")}
                  </p>
                </div>
              );
            })}
          ...
          {canReject ? (
            <div>
              <label
                htmlFor="cultural-event-image-translation-rejection-reason"
                className="block text-sm font-semibold text-slate-800"
              >
                Alasan penolakan <span className="text-red-700">*</span>
              </label>
              <textarea
                id="cultural-event-image-translation-rejection-reason"
