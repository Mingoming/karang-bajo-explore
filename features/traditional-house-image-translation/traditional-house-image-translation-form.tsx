...
import { manageTraditionalHouseImageTranslation } from "./actions";
import {
  getTraditionalHouseImageTranslationLifecycleLabel,
  isTraditionalHouseImageTranslationEditable,
  type TraditionalHouseImageTranslationActionState,
  type TraditionalHouseImageTranslationField,
  type TraditionalHouseImageTranslationSource,
} from "./model";

type Props = {
  traditionalHouseId: string;
  initialState: TraditionalHouseImageTranslationActionState;
  sourceReference: TraditionalHouseImageTranslationSource;
};

const FIELD_CONFIG: Array<{
  field: TraditionalHouseImageTranslationField;
  label: string;
  rows: number | null;
}> = [
...
function statusClasses(
  state: TraditionalHouseImageTranslationActionState["lifecycleState"],
) {
  if (state === "published")
...
export function TraditionalHouseImageTranslationForm({
  traditionalHouseId,
  initialState,
  sourceReference,
}: Props) {
  const boundAction = manageTraditionalHouseImageTranslation.bind(
    null,
    traditionalHouseId,
    sourceReference.id,
  );
  ...
  const lifecycleLabel = getTraditionalHouseImageTranslationLifecycleLabel(
    state.lifecycleState,
  );
  const editable = isTraditionalHouseImageTranslationEditable(
    state.status,
    state.reviewState,
  );
  const canReview =
    ...
    <section className="mt-8 border-t border-slate-200 pt-8">
      ...
      {state.lifecycleState === "source-blocked" ? (
        <div
          ...
          className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950"
        >
          Sumber rumah adat belum memenuhi kelayakan publik. Gambar Inggris
          tetap disembunyikan sampai sumber dipulihkan dan lolos database.
        </div>
      ) : null}
        ...
        >
          ...
          >
            ...
            {FIELD_CONFIG.map(({ field, label, rows }) => {
              const messageId = `traditional-house-image-translation-${field}-message`;
              const required = field === "alt_text";
              return (
                <div key={field}>
                  <label
                    htmlFor={`traditional-house-image-translation-${field}`}
                  ...
                  {rows === null ? (
                    <input
                      id={`traditional-house-image-translation-${field}`}
                      name={field}
                      type="text"
          ...
          {canReject ? (
            <div>
              <label
                htmlFor="traditional-house-image-translation-rejection-reason"
                className="block text-sm font-semibold text-slate-800"
              >
                Alasan penolakan{" "}
                <span aria-hidden="true" className="text-red-700">
              ...
              <textarea
                id="traditional-house-image-translation-rejection-reason"
