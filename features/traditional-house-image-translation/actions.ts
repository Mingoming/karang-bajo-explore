...
import {
  queryTraditionalHouseImageTranslationAdminData,
  type TraditionalHouseImageTranslationAdminReadResult,
} from "./data";
import {
  ...
  type TraditionalHouseImageTranslationActionState,
  type TraditionalHouseImageTranslationMutationValues,
  type TraditionalHouseImageTranslationRpcRow,
} from "./model";
...
type SuccessfulRead = Extract<
  TraditionalHouseImageTranslationAdminReadResult,
  { success: true }
>;
...
type StateOverrides = {
  kind?: TraditionalHouseImageTranslationActionState["kind"];
  values?: TraditionalHouseImageTranslationActionState["values"];
  fieldErrors?: TraditionalHouseImageTranslationActionState["fieldErrors"];
  formErrors?: string[];
  message?: string | null;
  rejectionReason?: string;
  sourceStatus?: TraditionalHouseImageTranslationActionState["sourceStatus"];
};
...
function stateFromRead(
  ...
  return createTraditionalHouseImageTranslationActionState(
    image.source,
    image.translation,
    image.history,
    {
...
function databaseFailureState(
  previousState: TraditionalHouseImageTranslationActionState,
  message = "Terjemahan gambar rumah adat belum dapat diproses. Silakan coba lagi.",
  kind: TraditionalHouseImageTranslationActionState["kind"] = "database-error",
) {
  return {
    ...previousState,
    kind,
...
function rpcFailureState(
  ...
  let message =
    "Terjemahan gambar rumah adat belum dapat diproses. Silakan coba lagi.";
  let kind: TraditionalHouseImageTranslationActionState["kind"] =
    "database-error";
  if (code === "55000") {
    kind = "conflict";
    message =
...
function revalidateTraditionalHouseImageTranslationPaths(
...
async function refreshAfterMutation(
  ...
  revalidateTraditionalHouseImageTranslationPaths(
    traditionalHouseId,
    trustedPreMutationSlug,
  );
  const refreshed = await queryTraditionalHouseImageTranslationAdminData(
    supabase,
    traditionalHouseId,
  );
  ...
  if (refreshed.slug !== trustedPreMutationSlug) {
    revalidateTraditionalHouseImageTranslationDetailPath(refreshed.slug);
  }
...
function rpcRowFailureCode(
  ...
  row: TraditionalHouseImageTranslationRpcRow | null,
) {
...
async function saveDraft(
  ...
  return supabase
    .rpc("traditional_house_image_translation_save_draft", {
      p_traditional_house_image_id: imageId,
      p_expected_edit_revision: expectedEditRevision,
      p_alt_text: values.alt_text,
      p_caption: values.caption,
    })
    .single()
    .overrideTypes<TraditionalHouseImageTranslationRpcRow, { merge: false }>();
}
...
export async function manageTraditionalHouseImageTranslation(
  traditionalHouseId: string,
  imageId: string,
  previousState: TraditionalHouseImageTranslationActionState,
  formData: FormData,
): Promise<TraditionalHouseImageTranslationActionState> {
  ...
  if (
    !isValidTraditionalHouseId(traditionalHouseId) ||
    !isValidTraditionalHouseId(imageId)
  ) {
    return databaseFailureState(
      previousState,
      "Rumah adat atau gambar tidak ditemukan.",
      "not-found",
    );
  }
  ...
  const current = await queryTraditionalHouseImageTranslationAdminData(
    supabase,
    traditionalHouseId,
  );
  if (!current.success) {
    return databaseFailureState(
      ...
      current.kind === "not-found"
        ...
        : "Terjemahan gambar rumah adat belum dapat dimuat. Silakan coba lagi.",
  ...
  if (!image) {
    return databaseFailureState(
      previousState,
      "Gambar tidak ditemukan atau bukan bagian dari rumah adat ini.",
      "not-found",
    );
  }
  ...
  if (intent === "save-draft" || intent === "review") {
    ...
    const sourceRule = validateTraditionalHouseImageTranslationForSource(
      image.source,
      validation.data,
    );
    ...
    if (intent === "save-draft") {
      return refreshAfterMutation(
        ...
        current.slug,
        previousState,
        "Draf terjemahan gambar rumah adat berhasil disimpan.",
      );
    }
    const reviewed = await supabase
      .rpc("traditional_house_image_translation_review", {
      ...
      .single()
      .overrideTypes<
        TraditionalHouseImageTranslationRpcRow,
        { merge: false }
      >();
    ...
    return refreshAfterMutation(
      ...
      current.slug,
      previousState,
      "Terjemahan gambar rumah adat berhasil dikirim untuk review.",
    );
  }
  ...
  if (intent === "reject") {
    ...
    const rejected = await supabase
      .rpc("traditional_house_image_translation_reject", {
    ...
    return refreshAfterMutation(
      ...
      current.slug,
      previousState,
      "Terjemahan gambar rumah adat dikembalikan menjadi draf dengan alasan penolakan.",
    );
  }
  if (intent === "publish" || intent === "republish") {
    ...
    return refreshAfterMutation(
      ...
      previousState,
      intent === "publish"
        ? "Terjemahan gambar rumah adat berhasil diterbitkan."
        : "Terjemahan gambar rumah adat berhasil diterbitkan kembali.",
    );
  }
  ...
  return refreshAfterMutation(
    ...
    previousState,
    intent === "archive"
      ? "Terjemahan gambar rumah adat berhasil diarsipkan."
      : intent === "unpublish"
        ? "Publikasi terjemahan gambar rumah adat dibatalkan dan kembali menjadi draf."
        : "Terjemahan gambar rumah adat dipulihkan menjadi draf.",
  );
}
