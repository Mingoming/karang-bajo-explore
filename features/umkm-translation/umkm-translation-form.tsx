...
import { manageUmkmTranslation } from "./actions";
import {
  getUmkmTranslationLifecycleLabel,
  isUmkmTranslationEditable,
  type UmkmTranslationActionState,
  type UmkmTranslationField,
  type UmkmTranslationSource,
} from "./model";

type Props = {
  initialState: UmkmTranslationActionState;
  sourceReference: UmkmTranslationSource;
};

const FIELD_CONFIG: Array<{
  field: UmkmTranslationField;
  label: string;
  rows: number | null;
}> = [
  { field: "business_name", label: "Nama usaha bahasa Inggris", rows: null },
  { field: "category", label: "Kategori usaha bahasa Inggris", rows: null },
  { field: "description", label: "Deskripsi bahasa Inggris", rows: 6 },
  { field: "address", label: "Alamat bahasa Inggris", rows: 3 },
];

const SOURCE_FIELD_LABELS: Record<UmkmTranslationField, string> = {
  business_name: "Nama usaha Indonesia",
  category: "Kategori usaha Indonesia",
  description: "Deskripsi Indonesia",
  address: "Alamat Indonesia",
};

function statusClasses(state: UmkmTranslationActionState["lifecycleState"]) {
  if (state === "published")
...
export function UmkmTranslationForm({ initialState, sourceReference }: Props) {
  const boundAction = manageUmkmTranslation.bind(null, sourceReference.id);
  ...
  const lifecycleLabel = getUmkmTranslationLifecycleLabel(state.lifecycleState);
  const editable = isUmkmTranslationEditable(state.status, state.reviewState);
  const canReview =
    editable &&
    (state.status === null ||
      (state.status === "draft" &&
        ...
        >
          ...
          >
            ...
            {FIELD_CONFIG.map(({ field, label, rows }) => {
              const messageId = `umkm-translation-${field}-message`;
              return (
                <div key={field}>
                  <label
                    htmlFor={`umkm-translation-${field}`}
          ...
          {canReject ? (
            <div>
              <label
                htmlFor="umkm-translation-rejection-reason"
                className={labelClasses}
              >
                Alasan penolakan{" "}
              ...
              <textarea
                id="umkm-translation-rejection-reason"
