...
Unauthenticated object URLs without a signed token or Supabase API authorization remain unavailable because the bucket is private. Administrator access is enforced by Storage policies through `public.is_admin()`. A single federated anonymous Storage `SELECT` policy verifies that an object is referenced by an image row whose destination, tourism package, homestay, UMKM, traditional house, or cultural event parent is published. The server batches trusted database paths through one closed-allowlist signer and creates signed URLs with a 600-second TTL. Because Supabase uses the same `SELECT` privilege for signing and download, a caller with the public API key and an exact eligible path can also fetch those published bytes directly; this does not extend to draft, archived, orphaned, malformed, or cross-entity objects. Paths used by the application originate from public-safe database rows, never visitor input, and signed URLs are never persisted. Signed public images bypass the Next.js image optimizer so its cache cannot outlive the signed URL. Public list, detail, and homepage consumers now use this boundary for all six parent types. The federated migration is locally validated and still requires hosted application and credential-backed validation.
...
Milestone 1 provides the public route-group shell, responsive navigation, footer, reusable presentation primitives, and homepage composition. Milestone 2 adds destination browsing. Milestone 3 establishes the locally validated federated public-media boundary. Milestone 4 adds published-only list and detail routes for tourism packages, homestays, UMKM, traditional houses, and cultural events, plus small deterministic homepage collections for all six domains. Public-safe views remain the read boundary, consent-filtered fields remain authoritative, and package itineraries expose only published related destinations. GIS, maps, cultural articles, global gallery, and other public modules remain subsequent work.
